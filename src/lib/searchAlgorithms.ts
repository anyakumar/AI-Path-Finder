...
export const algorithms = {
  bfs: { name: 'BFS', fn: bfs, type: 'Uninformed' as const, description: 'Explores all directions evenly and guarantees the shortest path in unweighted grids.' },
  dfs: { name: 'DFS', fn: dfs, type: 'Uninformed' as const, description: 'Dives deep into one path before backtracking, not guaranteed to find the shortest path.' },
  ucs: { name: 'Dijkstra', fn: ucs, type: 'Uninformed' as const, description: 'Finds the shortest path by exploring nodes based on minimum distance.' },
  greedy: { name: 'Greedy Best-First', fn: greedy, type: 'Informed' as const, description: 'Chooses paths that appear closest to the goal but may not be optimal.' },
  astar: { name: 'A*', fn: aStar, type: 'Informed' as const, description: 'Uses heuristics to efficiently find the shortest path toward the target.' },
};
